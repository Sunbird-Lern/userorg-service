...
/**
 * Enum defining various actor operations for User and Organization services.
 * Each operation consists of a string value (used in request/event processing)
 * and an operation code (used for auditing/tracking).
 */
public enum ActorOperations {
  ...
  /**
   * Returns the operation code associated with the operation.
   *
  ...
  /**
   * Retrieves the operation code for a given actor operation string value.
   *
   * @param actorOperation The string value of the operation.
   * @return The operation code if found, otherwise an empty string.
   */
  public static String getOperationCodeByActorOperation(String actorOperation) {
    ...
    return StringUtils.isNotBlank(opCode) ? opCode : "";
  }
}
