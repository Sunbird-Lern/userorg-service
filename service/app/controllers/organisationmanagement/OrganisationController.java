...
public class OrganisationController extends BaseController {
    ...
    public Promise<Result> createOrg() {
        ...
        try {
            ...
            ProjectLogger.log("Create organisation request: " + requestData,
            ...
            Map<String, Object> address =
                    (Map<String, Object>) reqObj.getRequest().get(JsonKey.ADDRESS);
            ...
            if (orgData.containsKey(JsonKey.PROVIDER)) {
                orgData.put(JsonKey.PROVIDER,
                        ((String) orgData.get(JsonKey.PROVIDER)).toLowerCase());
            }
            if (orgData.containsKey(JsonKey.EXTERNAL_ID)) {
                orgData.put(JsonKey.EXTERNAL_ID,
                        ((String) orgData.get(JsonKey.EXTERNAL_ID)).toLowerCase());
            }
    ...
    public Promise<Result> approveOrg() {
        ...
        try {
            JsonNode requestData = request().body().asJson();
            ProjectLogger.log("Approve organisation request: " + requestData,
                    LoggerEnum.INFO.name());
            Request reqObj = (Request) mapper.RequestMapper.mapRequest(requestData, Request.class);
            ProjectUtil.updateMapSomeValueTOLowerCase(reqObj);
    ...
    public Promise<Result> updateOrg() {
        ...
        try {
            JsonNode requestData = request().body().asJson();
            ProjectLogger.log("Update organisation request: " + requestData,
                    LoggerEnum.INFO.name());
            Request reqObj = (Request) mapper.RequestMapper.mapRequest(requestData, Request.class);
            ProjectUtil.updateMapSomeValueTOLowerCase(reqObj);
    ...
    public Promise<Result> getOrgDetails() {
        ...
        try {
            JsonNode requestData = request().body().asJson();
            ProjectLogger.log("Get Organisation details request: " + requestData,
                    LoggerEnum.INFO.name());
            Request reqObj = (Request) mapper.RequestMapper.mapRequest(requestData, Request.class);
            ProjectUtil.updateMapSomeValueTOLowerCase(reqObj);
            RequestValidator.validateOrg(reqObj);
    ...
    public Promise<Result> addMemberToOrganisation() {
        ...
        try {
            JsonNode requestData = request().body().asJson();
            ProjectLogger.log(" add member to organisation = " + requestData,
                    LoggerEnum.INFO.name());
            Request reqObj = (Request) mapper.RequestMapper.mapRequest(requestData, Request.class);
            ProjectUtil.updateMapSomeValueTOLowerCase(reqObj);
    ...
    public Promise<Result> approveUserOrganisation() {
        ...
        try {
            JsonNode requestData = request().body().asJson();
            ProjectLogger.log(" approve user organisation = " + requestData,
                    LoggerEnum.INFO.name());
            Request reqObj = (Request) mapper.RequestMapper.mapRequest(requestData, Request.class);
            ProjectUtil.updateMapSomeValueTOLowerCase(reqObj);
            RequestValidator.validateUserOrg(reqObj);
    ...
    public Promise<Result> search() {
        try {
            ...
            if (reqObj.getRequest().containsKey(JsonKey.FILTERS)
                ...
                ((Map) (reqObj.getRequest().get(JsonKey.FILTERS))).put(JsonKey.OBJECT_TYPE,
                        esObjectType);
            } else {
