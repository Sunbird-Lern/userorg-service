...
public class BadgeIssuerController extends BaseController {
  ...
  public Promise<Result> createBadgeIssuer() {
    try {
      ...
      reqObj = setExtraParam(reqObj, ExecutionContext.getRequestId(), BadgeOperations.createBadgeIssuer.name(),
   ...
   */
	public Promise<Result> getBadgeIssuer(String issuerId) {
		try {
			Request reqObj = new Request();
			reqObj = setExtraParam(reqObj, ExecutionContext.getRequestId(), BadgeOperations.getBadgeIssuer.name(),
					ctx().flash().get(JsonKey.USER_ID), getEnvironment());
  ...
  public Promise<Result> getAllIssuer() {
    try {
      Request reqObj = new Request();
      reqObj = setExtraParam(reqObj, ExecutionContext.getRequestId(), BadgeOperations.getAllIssuer.name(),
				ctx().flash().get(JsonKey.USER_ID), getEnvironment());
      return actorResponseHandler(getActorRef(), reqObj, timeout, null, request());
    } catch (Exception e) {
      return Promise.<Result>pure(createCommonExceptionResponse(e, request()));
  ...
  public Promise<Result> deleteBadgeIssuer(String issuerId) {
    try {
      Request reqObj = new Request();
      reqObj = setExtraParam(reqObj, ExecutionContext.getRequestId(), BadgeOperations.deleteIssuer.name(),
				ctx().flash().get(JsonKey.USER_ID), getEnvironment());
      reqObj.getRequest().put(JsonKey.SLUG , issuerId);
      BadgeIssuerRequestValidator.validateGetBadgeIssuerDetail(reqObj);
      return actorResponseHandler(getActorRef(), reqObj, timeout, null, request());
  ...
  /**
   * This method will read from input data and put all the 
   * requested key and value inside a map.
   * @param body MultipartFormData
   * @param map Map<String,Object>
   * @return Map<String,Object>
   * @throws IOException
   */
  private Map<String,Object> readFormData (MultipartFormData body , Map<String,Object> map) throws IOException{
