	...
	public Promise<Result> save() {
		...
		try {
			...
			if (isGzipRequest(request().headers())) {
				...
				ProjectLogger.log("Receiving telemetry in json format.", requestData, LoggerEnum.DEBUG.name());
				request = (Request) RequestMapper.mapRequest(requestData, Request.class);
			}
			request = setExtraParam(request, ExecutionContext.getRequestId(), ActorOperations.SAVE_TELEMETRY.getValue(),
