...
/**
 * This class will handle all the method related to authentication. For example
 * verifying user access token, creating access token after success login.
 *
 * @author Manzarul
 */
public class AuthenticationHelper {
	...
	private static boolean ssoEnabled = (StringUtils.isNotBlank(System.getenv(JsonKey.SSO_PUBLIC_KEY))
	...
	/**
	 * This method will verify the incoming user access token against store data
	 * base /cache. If token is valid then it would be associated with some user id.
	 * In case of token matched it will provide user id. else will provide empty
	 * string.
	 *
	 * @param token
	 *            String
	...
	public static String verifyUserAccesToken(String token) {
		...
		try {
			if (ssoEnabled) {
				...
				Response authResponse = cassandraOperation.getRecordById(userAuth.getKeySpace(),
						userAuth.getTableName(), token);
				if (authResponse != null && authResponse.get(JsonKey.RESPONSE) != null) {
					List<Map<String, Object>> authList = (List<Map<String, Object>>) authResponse.get(JsonKey.RESPONSE);
	...
	public static String verifyClientAccessToken(String clientId, String clientToken) {
		...
		try {
			Response clientResponse = cassandraOperation.getRecordsByProperties(clientDbInfo.getKeySpace(),
					clientDbInfo.getTableName(), propertyMap);
			if (null != clientResponse && !clientResponse.getResult().isEmpty()) {
				List<Map<String, Object>> dataList = (List<Map<String, Object>>) clientResponse.getResult()
	...
	public static Map<String, Object> getClientAccessTokenDetail(String clientId) {
		...
		propertyMap.put(JsonKey.ID, clientId);
		try {
			Response clientResponse = cassandraOperation.getRecordById(clientDbInfo.getKeySpace(),
					clientDbInfo.getTableName(), clientId);
			if (null != clientResponse && !clientResponse.getResult().isEmpty()) {
				List<Map<String, Object>> dataList = (List<Map<String, Object>>) clientResponse.getResult()
						.get(JsonKey.RESPONSE);
	...
	public static Map<String, Object> getUserDetail(String userId) {
		...
		Map<String, Object> response = null;
		try {
			Response userResponse = cassandraOperation.getRecordById(userDbInfo.getKeySpace(),
					userDbInfo.getTableName(), userId);
			if (null != userResponse && !userResponse.getResult().isEmpty()) {
				List<Map<String, Object>> dataList = (List<Map<String, Object>>) userResponse.getResult()
						.get(JsonKey.RESPONSE);
				response = dataList.get(0);
			}
		} catch (Exception e) {
