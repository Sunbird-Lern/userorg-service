...
public class TenantMigrationControllerTest extends BaseApplicationTest {
    ...
    public void testMigrationUserSuccess() {
        Result result = TestUtil.performTest("/v1/user/migrate", "POST", getSuccessMigrationReq(),application);
    ...
    public void tesPrivatetMigrationUserSuccess() {
        Result result = TestUtil.performTest("/private/user/v1/migrate", "PATCH", getSuccessMigrationReq(),application);
        assertEquals( ResponseCode.success.getErrorCode().toLowerCase(),TestUtil.getResponseCode(result));
    }
