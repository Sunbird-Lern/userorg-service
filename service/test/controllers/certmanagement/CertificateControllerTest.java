...
public class CertificateControllerTest extends BaseApplicationTest {
    ...
    public void tesCertificateDownloadSuccess() {
        Result result = TestUtil.performTest("/v1/user/certs/download", "POST", getRequest(),application);
        System.out.println("dfkd"+Helpers.contentAsString(result));
        assertEquals(ResponseCode.success.getErrorCode().toLowerCase(),TestUtil.getResponseCode(result));
    }

    @Test
    public void testCertificateDownloadFailure() {
        Result result = TestUtil.performTest("/v1/user/certs/download", "POST", getFailureReq(),application);
    ...
    public void tesCertificateValidateSuccess() {
        Result result = TestUtil.performTest("/private/user/v1/certs/validate", "POST", getValidateReq(null),application);
        System.out.println("dfkd"+Helpers.contentAsString(result));
        assertEquals(ResponseCode.success.getErrorCode().toLowerCase(),TestUtil.getResponseCode(result));
    }
    @Test
    public void tesCertificateValidateFailure() {
        Result result = TestUtil.performTest("/private/user/v1/certs/validate", "POST", getValidateReq(JsonKey.CERT_ID),application);
        System.out.println("dfkd"+Helpers.contentAsString(result));
        assertEquals(ResponseCode.mandatoryParamsMissing.getErrorCode(),TestUtil.getResponseCode(result));
    }
