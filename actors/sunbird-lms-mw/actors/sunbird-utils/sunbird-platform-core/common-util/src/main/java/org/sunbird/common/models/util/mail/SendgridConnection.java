...
public class SendgridConnection {
  ...
  private static Transport createConnection() {
    try {
      ...
      ProjectLogger.log(
          "Exception occurred while smtp session and creating transport connection", e);
    }
    return null;
  }
  ...
  private static boolean sendEmail(
    ...
    try {
      ...
      if (null == transport
        ...
        ProjectLogger.log(
            "SMTP Transport client connection is closed or timed out. Create new connection.",
            LoggerEnum.INFO.name());
        createConnection();
      }
  ...
  public static void sendMail(Map<String, Object> request, List<String> emails, String template) {
    try {
      Velocity.init();
      ...
      boolean status =
          sendEmail(
              ...
              session,
              transport);
      ProjectLogger.log("Email sent : " + status, LoggerEnum.INFO.name());
    } catch (Exception e) {
      ProjectLogger.log(
