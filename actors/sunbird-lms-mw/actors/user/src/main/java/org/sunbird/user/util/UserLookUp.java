    ...
    if (CollectionUtils.isNotEmpty(user.getExternalIds())) {
      for (Map<String, String> externalId : user.getExternalIds()) {
        if (StringUtils.isNotBlank(externalId.get(JsonKey.ID))
          ...
          String externalIdWithOrg = externalId.get(JsonKey.ID) +"@"+ externalId.get(JsonKey.PROVIDER);
          List<Map<String, Object>> externalIdsRecord = getRecordByType(JsonKey.EXTERNALID, externalIdWithOrg, false, context);
