                        ...
                        userId -> {
                            ...
                            if (CollectionUtils.isNotEmpty(userList) && userList.size() > 0) {
                                ...
                                if(CollectionUtils.isNotEmpty(usr.getLocationIds())){
                                    Map<String, Object> searchOrgQueryMap = new HashMap<>();
                                    searchOrgQueryMap.put(JsonKey.USER_ID, userId);
                                    ...
                                    List<Organisation> suborgDetailsList =
                                            organisationClient.esSearchOrgByIds(
                                                    suborgList, fields, context);

                                    boolean locationExists = false;
                                    List<String> subOrgLocationIds = null;
                                    for(Organisation org: suborgDetailsList){
                                        if(org.getLocationIds()!= null && org.getLocationIds().containsAll(usr.getLocationIds())){
                                            locationExists = true;
                                        }else{
                                    ...
                                    if(!locationExists){
                                        if(subOrgLocationIds != null) {
                                            updateUserLocation(context, userRespList, finalDryRun, usr.getId(), subOrgLocationIds);
                                        }
                                    }
                                ...
                                logger.info(
                                        "For the given userid no user exist in ES. "
                                                + userId);
                            }
                        });
